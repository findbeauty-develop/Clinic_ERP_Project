...
export class SolapiProvider {
  ...
  async sendSMS(phoneNumber: string, message: string): Promise<boolean> {
    try {
      ...
      this.logger.debug(`Solapi response for ${formattedPhone}: ${JSON.stringify(result)}`);
      
      if (groupInfo) {
        // Check multiple success indicators
        const hasGroupId = !!groupInfo.groupId;
        const isSending = groupInfo.status === "SENDING" || groupInfo.status === "COMPLETE";
        ...
        if (hasGroupId || isSending || hasRegisteredSuccess || hasSentSuccess || hasDateSent) {
          this.logger.log(`SMS sent successfully to ${formattedPhone} (status: ${groupInfo.status}, registeredSuccess: ${groupInfo.count?.registeredSuccess})`);
          return true;
        }
      }
      ...
      if (result && (result as any).successCount > 0) {
        ...
        this.logger.warn(`SMS send failed to ${formattedPhone}: ${JSON.stringify(result)}`);
        return false;
      }
    } catch (error: any) {
