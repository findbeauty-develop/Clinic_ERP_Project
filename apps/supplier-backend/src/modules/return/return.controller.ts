...
export class ReturnController {
  ...
  @ApiQuery({
    ...
    enum: ["반품", "교환"],
    description:
      "DEPRECATED: Use returnCategory instead. Filter by return type: '반품' (return) or '교환' (exchange)",
  })
  @ApiQuery({
    name: "returnCategory",
    required: false,
    ...
    description:
      "Filter by return category: 'empty_box' (빈 박스 반납) or 'product' (제품 반품/교환)",
  })
  async getReturnNotifications(
    ...
    return this.returnService.getReturnNotifications(supplierManagerId, {
      status: status || "ALL",
      returnType: returnType, // DEPRECATED: for backward compatibility
      returnCategory: returnCategory, // NEW: filter by category (empty_box or product)
  ...
  async acceptReturn(
    ...
    return this.returnService.acceptReturn(
      id,
      supplierManagerId,
      body.itemId,
      body.adjustments
  ...
  @ApiOperation({
    ...
    description:
      "Return'ni completed deb belgilash va clinic'ga xabar yuborish",
  })
  ...
  @ApiOperation({
    summary: "Clinic → Supplier return request yaratish (API Key auth)",
  })
  @ApiHeader({
    name: "x-api-key",
