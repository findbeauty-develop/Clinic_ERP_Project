...
export class SolapiProvider implements IMessageProvider {
  ...
  async sendSMS(phoneNumber: string, message: string, isCritical: boolean = false): Promise<boolean> {
    try {
      ...
      if (groupInfo) {
        ...
        if (
          groupInfo.groupId ||
          groupInfo.status === "SENDING" ||
          groupInfo.count?.registeredSuccess > 0
        ) {
          this.logger.log(
            `Solapi SMS sent to ${phoneNumber} (${formattedPhone})`
          );
          return true;
        }
      }
      ...
      if (error) {
        ...
        if (Object.keys(errorProps).length > 0) {
          this.logger.error(
            `Error properties: ${JSON.stringify(errorProps, null, 2)}`
          );
        }
      }
  ...
  async sendKakaoTalk(phoneNumber: string, message: string): Promise<boolean> {
    try {
      ...
      if (!pfId || !templateId) {
        this.logger.warn(
          "Solapi KakaoTalk credentials not configured (pfId or templateId missing)"
        );
        return false;
      }
      ...
      if (result && (result as any).successCount > 0) {
        this.logger.log(
          `Solapi KakaoAlimTalk sent to ${phoneNumber} (${formattedPhone})`
        );
        return true;
      } else if (result && (result as any).groupId) {
        // Alternative: if groupId exists, message was sent
        ...
        this.logger.warn(
          `Solapi KakaoAlimTalk failed: ${JSON.stringify(result)}`
        );
        return false;
      }
    } catch (error: any) {
      ...
      if (error?.response?.data) {
        this.logger.error(
          `Solapi API error response: ${JSON.stringify(error.response.data, null, 2)}`
        );
