...
export class KTCommunisProvider implements IMessageProvider {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.apiSecret = this.configService.get<string>('KT_COMMUNIS_API_SECRET') || '';
    this.fromNumber = this.configService.get<string>('KT_COMMUNIS_FROM_NUMBER') || '';
    this.kakaoTemplateCode = this.configService.get<string>('KT_COMMUNIS_KAKAO_TEMPLATE_CODE');
  }
  ...
  async sendSMS(phoneNumber: string, message: string): Promise<boolean> {
    try {
      ...
      const response = await fetch('https://api.communis.kt.co.kr/sms/v1/send', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`KT Communis SMS error: ${response.statusText} - ${errorText}`);
      }
  ...
  async sendKakaoTalk(phoneNumber: string, message: string): Promise<boolean> {
    try {
      ...
      const response = await fetch('https://api.communis.kt.co.kr/kakao/v1/send', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.apiKey}`, // yoki KT Communis'ning auth format'i
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`KT Communis KakaoTalk error: ${response.statusText} - ${errorText}`);
      }
