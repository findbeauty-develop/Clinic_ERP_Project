  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async executeWithRetry<T>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (isConnectionError && attempt < retries) {
          this.logger.warn(
            `Database connection error (attempt ${attempt + 1}/${
              retries + 1
          ...
          if (
            ...
            try {
              ...
              this.logger.debug(
                "Reconnect attempt failed, will retry operation"
              );
            }
          }
    ...
    this.logger.error(
      `Database operation failed after ${retries + 1} attempts. Last error: ${
  ...
  private async connectWithRetry(
    ...
    if (retryCount >= maxAttempts) {
      this.logger.error(
        `Max retry attempts (${maxAttempts}) reached. Stopping connection attempts.`
      );
      throw new Error(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to connect to database (attempt ${retryCount + 1}/${
          this.maxRetries
        }): ${errorMessage}`
      );
      ...
      if (process.env.DATABASE_URL) {
        try {
          ...
          this.logger.warn(
            `Attempting to connect to: ${url.hostname}:${url.port || 5432}`
          );
        } catch (e) {
      ...
      if (process.env.DATABASE_URL) {
        try {
          ...
          if (url.port === "5432" && url.hostname.includes("supabase")) {
            this.logger.error(
              "💡 Tip: Try changing port to 6543 and add ?pgbouncer=true"
            );
          }
        } catch (e) {
          // URL parsing failed
