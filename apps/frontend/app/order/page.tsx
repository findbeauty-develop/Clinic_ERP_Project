...
export default function OrderPage() {
  ...
  useEffect(() => {
    ...
    const handlePartialInboundCompleted = () => {
      if (activeTab === "history") {
        fetchOrders();
      }
    };
    ...
    window.addEventListener(
      "partialInboundCompleted",
      handlePartialInboundCompleted
    );
  ...
  const handleSavePrices = useCallback(async () => {
    ...
    try {
      ...
      // Add to order immediately with updated product
      const itemId = selectedBatchId
        ? `${selectedProduct.id}-${selectedBatchId}`
        : selectedProduct.id;
      const supplierId = selectedProduct.supplierId || "unknown";

      // Update quantities state
      setQuantities((prev) => ({
        ...prev,
        [selectedProduct.id]: pendingQuantity,
        [itemId]: pendingQuantity,
      }));
      ...
      alert("가격이 저장되고 주문에 추가되었습니다");
    } catch (error: any) {
      console.error("Failed to save prices:", error);
      alert(error?.message || "가격 저장 실패");
    }
  }, [selectedProduct, selectedBatchId, pendingQuantity, modalPrices, apiUrl]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Search filter: 제품명, 브랜드, 카테고리, 공급업체
    const searchTerm = debouncedProductSearchQuery.trim().toLowerCase();
    const searchFiltered = searchTerm
      ? productsWithCalcs.filter((product) => {
          return (
    ...
    // Filter based on productFilter state
    const filtered =
      productFilter === "low-stock"
        ? searchFiltered.filter((product) => product.isLowStock)
        : searchFiltered;
    ...
    return sorted;
  }, [products, productFilter, debouncedProductSearchQuery]);
        ...
        {activeTab === "processing" ? (
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex w-2/3 flex-col overflow-hidden border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
              ...
              {/* 재고 부족 제품 / 전체 제품 Card - faqat search query bo'sh bo'lsa ko'rsatish */}
              {!debouncedOrderSearchQuery.trim() && (
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="mb-2 flex items-center gap-3 text-sm">
                                ...
                                >
                                  현재고: {product.currentStock || 0}{" "}
                                  {product.unit || "개"}
                                </span>
                                <span className="text-slate-600 dark:text-slate-400">
                                  최소재고: {product.minStock || 0}{" "}
                                  {product.unit || "개"}
                                </span>
                              </div>
          ...
          <div className="flex-1 p-6">
            ...
            {ordersLoading ? (
              ...
              <div className="space-y-4">
                {/* Regular orders: split by item status (주문 요청 / 주문 진행). Rejected orders only appear below after confirmation in 입고 대기. */}
                {(() => {
                  type CardItem = {
                    order: any;
                    sectionLabel: "주문 요청" | "주문 진행";
                  ...
                  orders
                    ...
                    .forEach((order) => {
                      ...
                      if (confirmedItems.length > 0) {
                        const cardOrder = { ...order, items: confirmedItems };
                        cardOrder.totalAmount = confirmedItems.reduce(
                          (sum: number, item: any) =>
                            sum +
                            (item.orderedQuantity || 0) * (item.unitPrice || 0),
                          0
                      ...
                      // Inbounded kartani har doim ko'rsatish (completed yoki pending_inbound bo'lganda)
                      if (
                        (isOrderCompleted ||
                        ...
                        // Faqat inbounded itemlar — rejected itemlar alohida "거절 주문" bo'limida
                  ...
                  return cards.map(
                    (
                      { order, sectionLabel, hasRejectedItems = false },
                      cardIdx
                    ) => {
                      ...
                      const isPending = sectionLabel === "주문 요청";

                      // Extract rejection reasons from items (this card's items only)
                      const rejectionReasons =
                        order.items
                          ?.map((item: any) => {
                            if (
                              item.memo &&
                              item.memo.includes("[거절 사유:")
                            ) {
                      ...
                      return (
                        <div
                          key={`${order.id}-${sectionLabel}-${cardIdx}`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg w-full max-w-2xl shadow-xl">
            ...
            <div className="p-6 space-y-4 gap-4">
              {/* Warning */}
              <div className="flex-1 mb-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <div className="flex gap-2 text-xs text-yellow-800 dark:text-yellow-300">
                  <span className="flex-shrink-0">⚠️</span>
                  <div>
                    가격 정보가 없습니다. 설정 시 제품에 저장되며 이후 주문에도
                    적용됩니다.
                  </div>
                </div>
              </div>
              <div className="flex items-center justify-between w-full gap-4">
                {/* Product Info */}
                <div className="flex-shrink-0 w- mt-4 p-3 ">
